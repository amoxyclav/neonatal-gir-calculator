...
(async()=>{
  ...
  try{
    ...
    async function chooseFluid(name){
      ...
      const option=picker.locator('.fluid-picker-option').filter({hasText:new RegExp('^'+name+'    if(await girPickerTrigger.count()!==1) throw new Error('GIR predefined-fluid picker trigger is missing.');
    ...
    if(await page.locator('#fluidPickerDialog').isVisible()!==true) throw new Error('Predefined-fluid picker dialog did not open.');

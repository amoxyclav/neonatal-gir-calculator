...
(async()=>{
  ...
  try{
    ...
    const pickerD10=page.locator('#fluidPickerDialog .fluid-picker-volume[data-fluid-name="D10"]');
    const d5Unit=page.locator('#fluidPickerDialog .fluid-picker-unit[data-fluid-name="D5"]');
    const d10Unit=page.locator('#fluidPickerDialog .fluid-picker-unit[data-fluid-name="D10"]');
    if(await d5Unit.count()!==1||await d10Unit.count()!==1) throw new Error('Each predefined fluid must have its own volume-unit selector.');
    ...
    await pickerD5.fill('24');
    await d5Unit.selectOption('mL/hr');
    if(await pickerD5.getAttribute('placeholder')!=='Volume (mL/hr)'||await pickerD5.inputValue()!=='1') throw new Error('Switching D5 to mL/hr should convert only its volume and update its placeholder.');
    if(await d10Unit.inputValue()!=='mL/day') throw new Error('D10 should retain its independent mL/day unit.');
    await d5Unit.selectOption('mL/day');
    if(await pickerD5.getAttribute('placeholder')!=='Volume (mL/day)'||await pickerD5.inputValue()!=='24') throw new Error('Switching D5 back to mL/day should preserve its equivalent volume.');
    ...
    await girPickerTrigger.click();
    await page.locator('#fluidPickerDialog .fluid-picker-unit[data-fluid-name="D5"]').selectOption('mL/hr');
    await page.locator('#fluidPickerDialog .fluid-picker-unit[data-fluid-name="D10"]').selectOption('mL/day');
    await page.locator('#fluidPickerDialog .fluid-picker-volume[data-fluid-name="D5"]').fill('1.5');
    await page.locator('#fluidPickerDialog .fluid-picker-volume[data-fluid-name="D10"]').fill('12');
    await page.locator('#fluidPickerDialog #applyPredefinedFluids').click();
    const hourlyD5=page.locator('#fluidList .fluid').filter({hasText:'D5'}).first();
    if(await page.locator('#currentTotal').textContent()!=='48.0') throw new Error('A D5 volume of 1.5 mL/hr plus D10 12 mL/day should total 48.0 mL/day.');
    ...
    const dailyD10=page.locator('#fluidList .fluid').filter({hasText:'D10'}).first();
    if(await dailyD10.locator('.volume-unit').inputValue()!=='mL/day'||await dailyD10.locator('.volume-input').inputValue()!=='12.0') throw new Error('D10 should retain its independent mL/day unit.');
    await hourlyD5.locator('.remove').click();
    await dailyD10.locator('.remove').click();

    await girPickerTrigger.click();
    await page.locator('#fluidPickerDialog .fluid-picker-unit[data-fluid-name="D5"]').selectOption('mL/day');
    ...
    await girPickerTrigger.click();
    const aaPicker=page.locator('#fluidPickerDialog .fluid-picker-conc[data-fluid-name="Aminoven"]');
    const lipidPicker=page.locator('#fluidPickerDialog .fluid-picker-conc[data-fluid-name="Intralipid"]');
    const isoPicker=page.locator('#fluidPickerDialog .fluid-picker-conc[data-fluid-name="Isolyte P"]');
    if(await aaPicker.inputValue()!=='10'||await lipidPicker.inputValue()!=='20'||await isoPicker.inputValue()!=='5') throw new Error('AA%, lipid concentration, and Isolyte P D% controls should load their preset defaults.');
    await aaPicker.fill('12');
    await lipidPicker.fill('15');
    await isoPicker.fill('4');
    for(const name of ['Aminoven','Intralipid','Isolyte P']) await page.locator('#fluidPickerDialog .fluid-picker-volume[data-fluid-name="'+name+'"]').fill('1');
    await page.locator('#fluidPickerDialog #applyPredefinedFluids').click();
    const aaFluid=page.locator('#fluidList .fluid').filter({has:page.locator('.fluid-name b').filter({hasText:/^Aminoven$/})}).first();
    const lipidFluid=page.locator('#fluidList .fluid').filter({has:page.locator('.fluid-name b').filter({hasText:/^Intralipid$/})}).first();
    const isoFluid=page.locator('#fluidList .fluid').filter({has:page.locator('.fluid-name b').filter({hasText:/^Isolyte P$/})}).first();
    await aaFluid.locator('.details-toggle').click();
    await lipidFluid.locator('.details-toggle').click();
    await isoFluid.locator('.details-toggle').click();
    if(await aaFluid.locator('[data-field="conc"]').inputValue()!=='12'||await aaFluid.locator('[data-field="protein"]').inputValue()!=='12'||await aaFluid.locator('[data-field="energy"]').inputValue()!=='48') throw new Error('Aminoven AA% should update protein and energy values.');
    if(await lipidFluid.locator('[data-field="conc"]').inputValue()!=='15'||await lipidFluid.locator('[data-field="energy"]').inputValue()!=='150') throw new Error('Intralipid concentration should update its energy value.');
    if(await isoFluid.locator('[data-field="conc"]').inputValue()!=='4'||await isoFluid.locator('[data-field="carb"]').inputValue()!=='4'||await isoFluid.locator('[data-field="energy"]').inputValue()!=='13.6') throw new Error('Isolyte P D% should update its carbohydrate and energy values.');
    if(await page.locator('#currentEnergy').textContent()!=='2.1'||await page.locator('#currentProtein').textContent()!=='0.12'||await page.locator('#currentCarb').textContent()!=='0.04') throw new Error('Picker concentration changes should update the GIR page nutrition cards.');
    if(await page.locator('#nutCurrentEnergy').textContent()!=='2.1'||await page.locator('#nutCurrentProtein').textContent()!=='0.12'||await page.locator('#nutCurrentCarb').textContent()!=='0.04') throw new Error('Picker concentration changes should update the Nutrition page summary cards.');
    await aaFluid.locator('[data-field="conc"]').fill('14');
    if(await aaFluid.locator('[data-field="energy"]').inputValue()!=='56'||await aaFluid.locator('[data-field="protein"]').inputValue()!=='14') throw new Error('Editing Aminoven contents after adding it should recalculate its derived values.');
    const aaId=await aaFluid.locator('[data-field="conc"]').getAttribute('data-id');
    if(await page.locator('[data-nfield="conc"][data-nid="'+aaId+'"]').inputValue()!=='14'||await page.locator('[data-nfield="energy"][data-nid="'+aaId+'"]').inputValue()!=='56') throw new Error('Editing a fluid on the GIR page should sync its editable composition on the Nutrition page.');
    if(await page.locator('[data-settings-product="Aminoven"][data-settings-key="conc"]').inputValue()!=='14'||await page.locator('[data-settings-product="Aminoven"][data-settings-key="energy"]').inputValue()!=='56') throw new Error('Fluid composition edits should update the matching preset values.');
    const savedEditablePresets=JSON.parse(await page.evaluate(()=>localStorage.getItem('neonatalGirEditablePresets.v1')||'{}'));
    if(savedEditablePresets.Aminoven?.conc!==14||savedEditablePresets.Aminoven?.energy!==56) throw new Error('Editing Aminoven should persist its preset composition in device storage.');
    await page.reload({waitUntil:'networkidle'});
    await page.locator('[data-page="gir"]').first().click();
    await girPickerTrigger.click();
    if(await page.locator('#fluidPickerDialog .fluid-picker-conc[data-fluid-name="Aminoven"]').inputValue()!=='14'||await page.locator('#fluidPickerDialog .fluid-picker-conc[data-fluid-name="Intralipid"]').inputValue()!=='15'||await page.locator('#fluidPickerDialog .fluid-picker-conc[data-fluid-name="Isolyte P"]').inputValue()!=='4') throw new Error('Edited preset concentrations should persist after reloading the calculator.');
    await page.locator('#fluidPickerDialog .fluid-picker-conc[data-fluid-name="Aminoven"]').fill('10');
    await page.locator('#fluidPickerDialog .fluid-picker-conc[data-fluid-name="Intralipid"]').fill('20');
    await page.locator('#fluidPickerDialog .fluid-picker-conc[data-fluid-name="Isolyte P"]').fill('5');
    await page.locator('#fluidPickerDialog #applyPredefinedFluids').click();
    while(await page.locator('#fluidList .fluid').count()) await page.locator('#fluidList .fluid .remove').first().click();

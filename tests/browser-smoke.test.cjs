...
(async()=>{
  ...
  try{
    ...
    if(await pickerD5.getAttribute('placeholder')!=='Volume (mL/hr)'||await pickerD5.inputValue()!=='1') throw new Error('Switching D5 to mL/hr should convert only its value and update its placeholder.');
    await d10Unit.selectOption('mL/day');
    if(await pickerD10.getAttribute('placeholder')!=='Volume (mL/day)') throw new Error('D10 should retain its independent mL/day unit.');
    ...
    await girPickerTrigger.click();
    await page.locator('#fluidPickerDialog [data-picker-unit="mL/day"]').click();

...
(async()=>{
  ...
  try{
    ...
    const pickerDay=page.locator('#fluidPickerDialog [data-picker-unit="mL/day"]');
    const pickerHour=page.locator('#fluidPickerDialog [data-picker-unit="mL/hr"]');
    if(await pickerDay.count()!==1||await pickerHour.count()!==1) throw new Error('Predefined-fluid picker must offer both mL/day and mL/hr units.');
    ...
    await pickerHour.click();
    if(await pickerD5.getAttribute('placeholder')!=='Volume (mL/hr)'||await pickerD5.inputValue()!=='1') throw new Error('Switching the picker to mL/hr should convert the entered value and update its placeholder.');
    await pickerDay.click();
    if(await pickerD5.getAttribute('placeholder')!=='Volume (mL/day)'||await pickerD5.inputValue()!=='24') throw new Error('Switching the picker back to mL/day should preserve the equivalent volume.');
    ...
    await girPickerTrigger.click();
    await page.locator('#fluidPickerDialog [data-picker-unit="mL/hr"]').click();
